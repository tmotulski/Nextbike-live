...
setwd("c:/Projects/Carsharing/nextbike-swarad/transform")
...
for (fileCount in seq_along(files)) {
  ...
  if (length(tmp_all_places2) > 0) {
  init_df <- data.frame(
    #num = NA,
    datetime = as.character(strptime(substr(filename, 4, 22), "%Y-%m-%dT%H_%M_%S")),
...
full_df$datetime <- as.POSIXct(strptime(full_df$datetime, "%Y-%m-%d %H:%M:%S"))
...
for (ii in 1:bikeGroupCount) {
  ...
  for (i in 1:bikeRCount) {
    ...
    #           start_datetime = bike_interval$end_datetime[i3],
    #           end_datetime = bike_interval$start_datetime[i3+1],
    #           country = bike_data$country[i],
    #           country_company = bike_data$country_company[i],
  ...
  full_route2 <- rbind (full_route2, bike_route)
...
test_bike_number = 92274
test_df <- full_df[full_df$bike_number == test_bike_number,]
test_interval <- full_interval[full_interval$bike_number == test_bike_number,]
test_route <- full_route[full_route$bike_number == test_bike_number,]
